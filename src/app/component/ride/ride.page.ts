...
import { FeedbekPage } from '../feedbek/feedbek.page';
import { PopoverComponent } from '../../popover/popover.component';
import { TimeRide } from 'src/app/modals/TimeRide';
import { UserRideService } from 'src/app/services/UserRide/user-ride.service';
...
export class RidePage implements OnInit {
  ...
  constructor(public route: Router,
    ...
    public userRideService: UserRideService,
  ...
  ngOnInit() {
    ...
    this.listChildren.forEach(element => {
      this.listChildren.push(element);
    });
  }
  ...
  async myListRides() {
    ...
    let userName;
    var options = {
      cssClass: 'my-custom-class', header: 'Hellow ' + this.currentUser.UserName, message: 'The rides in list', inputs: [], buttons: []
    };

    this.usersRideAvg.forEach(element => {
      this.rides.forEach(element1 => {
        if (element.RideId == element1.RideId) { ride = element1.RideName }
      });
      if (this.currentUser.UserId == element.UserId) { userName = this.currentUser.UserName }
      else {
        this.currentUser.Children.forEach(element2 => {
          if (element.UserId == element2.UserId) { userName = element2.UserName }
        });
      }
      options.inputs.push({
        cssClass: "alertMyRidesList", name: 'value1', type: 'button', value: ride + ":        " + userName
      })
    });
  ...
  async presentActionSheet(ride: Ride) {
    const actionSheet = await this.actionSheetController.create({
      header: 'Albums',
      cssClass: 'my-custom-class',
      buttons: [{
        text: 'Delete from list rides',
        role: 'destructive',
        icon: 'trash',
        handler: () => {
        ...
        text: 'Edit to rides list',
        icon: 'share',
        handler: () => {
          this.addToList(ride)
        }
      }, {
        text: 'See me my rides list',
        icon: 'eye-outline',
        handler: () => {
          this.myListRides();
  ...
  //     if (this.currentUser.Children.length > 0) { this.alertWithChildren(ride); }
  //     else {
...
// 
  ...
  //       }
  //     }
  //     console.log("xxx", this.listUsersRegister);
  ...
  //       console.log(this.rides[res[i].RideId - 1]);
  //       this.rides[rideId - 1].TimeEvening.push(timeRide);
